...
import { S } from "../i18n";
import { Button, DangerConfirm, Input, Dropdown } from "../ui";
import { toast } from "../toast";

/** op → 那句话里的动词。**数字与集合两类分开**，因为它们的操作数长得不一样 */
const OPS: { value: string; label: () => string; operand: "num" | "range" | "set" | "none" }[] = [
...
const operandKind = (op: string) => OPS.find((o) => o.value === op)?.operand ?? "num";
...
function parseOperand(op: string, text: string): unknown | undefined {
  ...
  if (kind === "range") {
    ...
    return parts.length === 2 && parts.every((n) => Number.isFinite(n)) ? parts : undefined;
  }
  const n = Number(t);
...
const emptyDraft = (classes: EntityTypeView[], attrs: RelationTypeView[]): Draft => ({
  name: "",
  description: "",
  ...
  conclude_value: "",
  conditions: attrs[0] ? [{ predicate_id: attrs[0].id, op: "gt", text: "" }] : [],
});
...
export function RulesPanel({
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return api.createRule(kbId, {
        ...
        conclude_type_id: d.conclusion === "typing" ? d.conclude_type_id : undefined,
        conclude_predicate_id:
          d.conclusion === "attribute" ? d.conclude_predicate_id : undefined,
        conclude_value: d.conclusion === "attribute" ? d.conclude_value : undefined,
        conditions,
      });
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        {/* 标题与「立刻跑」同一行，但**窄了就换行**：面板宽度跟着本体页左栏走，
            挤在一起时按钮会压到标题上 */}
        <div className="flex items-center gap-2 flex-wrap">
          <h2 className="text-sm font-bold text-neutral-200">{S.ontology.rulesTitle}</h2>
          {list.length > 0 && (
            <span className="u-num text-xs text-neutral-500">{list.length}</span>
          )}
          <Button
            size="sm"
            variant="ghost"
      ...
      {list.map((r) => (
        <div key={r.id} className="glass rounded-xl p-3.5">
          <div className="flex items-baseline gap-2">
            ...
            {r.derived_count > 0 && (
              <span className="u-num text-[10.5px] text-neutral-500">
                {S.ontology.ruleDerivedCount(r.derived_count)}
              </span>
            )}
            <button
              onClick={() => toggle.mutate(r)}
              className={`ml-auto shrink-0 u-chip !text-[10px] ${
                r.enabled ? "u-chip-accent" : "u-chip-neutral"
              }`}
            >
          ...
          <p className="mt-2 text-[12px] leading-relaxed text-neutral-300">
            <span className="text-neutral-500">{S.ontology.ruleSubject} </span>
            {r.subject_label}
            <span className="text-neutral-600"> ({S.ontology.ruleSubjectHint})</span>
            <span className="text-neutral-500">, {S.ontology.ruleConditions} </span>
            {r.conditions.map((c, i) => (
              <span key={i}>
                {i > 0 && <span className="text-neutral-600"> · </span>}
                <span className="text-neutral-200">{c.predicate_label}</span>{" "}
                <span className="text-neutral-500">
                  {OPS.find((o) => o.value === c.op)?.label() ?? c.op}
                </span>{" "}
                <span className="u-num text-neutral-200">
                  {operandText(c.op, c.operand)}
                </span>
              </span>
            ))}
            <span className="text-neutral-500"> → {S.ontology.ruleConcludes} </span>
      ...
      {!list.length && !draft && (
        <p className="text-xs text-neutral-500">{S.ontology.rulesEmpty}</p>
      )}
      ...
      {draft ? (
        <div className="glass rounded-xl p-3.5 space-y-2.5">
          ...
          <div className="flex items-center gap-2 text-[11px] text-neutral-500">
            <span className="shrink-0">{S.ontology.ruleSubject}</span>
            <Dropdown
          ...
          <div className="space-y-1.5">
            <div className="text-[11px] text-neutral-500">{S.ontology.ruleConditions}</div>
          ...
          <div className="flex items-center gap-2 text-[11px] text-neutral-500">
            <span className="shrink-0">{S.ontology.ruleConcludes}</span>
            <Dropdown
            ...
            {draft.conclusion === "typing" ? (
              ...
              <>
                ...
                <Input
                  ...
                  onChange={(e) => setDraft({ ...draft, conclude_value: e.target.value })}
