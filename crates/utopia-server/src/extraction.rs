...
async fn run(state: &AppState, document_id: Uuid) -> anyhow::Result<()> {
    ...
    let rel_meta: HashMap<Uuid, (bool, bool, String)> = rtypes
        .iter()
        .map(|r| (r.id, (r.functional, r.inverse_functional, r.temporal.clone())))
        .collect();
    ...
    for chunk in chunks.iter() {
        ...
        for f in &extraction.facts {
            ...
            if let Some(attr) = attr_hit {
                ...
                let Some(domain) = attr.domain_type_id else { continue };
                ...
                .await?;
                utopia_store::graph::add_evidence(&state.pool, fact_id, chunk.id, f.quote.as_deref())
                    .await?;
                if !created {
                    continue;
                }
