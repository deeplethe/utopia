...
impl AppState {
    ...
    pub fn emit_document(&self, kb_id: Uuid, document_id: Uuid) {
        let _ = self.events.send(AppEvent { kb_id, kind: "document", document_id: Some(document_id) });
    }

    pub fn emit_review(&self, kb_id: Uuid) {
        let _ = self.events.send(AppEvent { kb_id, kind: "review", document_id: None });
    }

    pub fn emit_source(&self, kb_id: Uuid) {
        let _ = self.events.send(AppEvent { kb_id, kind: "source", document_id: None });
    }
}
