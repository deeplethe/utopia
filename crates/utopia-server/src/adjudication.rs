...
pub async fn adjudicate_entities(state: &AppState, kb_id: Uuid) -> anyhow::Result<()> {
    ...
    let Some(client) = client else {
        ...
        let items = utopia_store::resolution::pending_adjudications(&state.pool, kb_id, 500).await?;
        for item in items {
    ...
    for _ in 0..MAX_ROUNDS {
        ...
        for item in items {
            ...
            match utopia_store::resolution::get_verdict(&state.pool, kb_id, &key).await? {
                Some((same, conf)) => apply_verdict(state, kb_id, &item, same, conf, "cached").await?,
            ...
            match by_i.get(&idx) {
                Some(v) => {
                    ...
                    utopia_store::resolution::put_verdict(
                        &state.pool, kb_id, key, same, conf, &model,
                    )
                    .await?;
...
async fn apply_verdict(
    ...
    match same {
        Some(true) if conf >= AUTO_CONF => {
            let (target, source) = utopia_store::resolution::merge_direction(
                &state.pool,
                item.left.id,
                item.right.id,
            )
            ...
            match utopia_store::resolution::merge_entities(
                &state.pool, kb_id, source, target, None, &reason,
            )
            .await
            {
                Ok(_) => {
                    utopia_store::resolution::close_review_auto(
                        &state.pool, item.id, "merged", &reason,
                    )
                    .await?;
