...
impl IngestAction {
    ...
    pub fn counts(self) -> bool {
        matches!(self, IngestAction::Created | IngestAction::Updated)
    }
}
        ...
        match fetch_page(&http, url).await {
            Ok((filename, mime, bytes)) => {
                ...
                let action = ingest_item(
                    state, source.kb_id, source.id, url, &filename, &mime, &bytes, None,
                )
                .await?;
                stats.absorb(action);
            }
...
fn filename_from_url(url: &str, mime: &str) -> String {
    ...
    let mut slug: String = stripped
        ...
        .collect();
    slug.truncate(120);
    let has_ext = slug.rsplit('.').next().map(|e| e.len() <= 5).unwrap_or(false)
        && slug.contains('.')
        && !slug.ends_with('.');
...
async fn sync_custom(state: &AppState, source: &Source) -> anyhow::Result<SyncStats> {
    ...
    let loopback = url
        .host_str()
        .map(|h| h.eq_ignore_ascii_case("localhost") || h == "127.0.0.1" || h == "::1" || h == "[::1]")
        .unwrap_or(false);
    ...
    if let Some(auth) = source.config["auth_header"].as_str().filter(|s| !s.trim().is_empty()) {
    ...
    for item in items.iter().take(MAX_NEW_PER_SYNC) {
        ...
        let title = item["title"].as_str().map(str::trim).filter(|s| !s.is_empty()).unwrap_or(id);
