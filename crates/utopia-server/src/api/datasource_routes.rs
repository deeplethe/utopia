...
pub async fn mount(
    ...
    let synced = sync_schema_doc(&state, kb_id, ds_id).await.map_err(AppError::Other)?;
...
pub async fn explore(
    ...
    require_kb(&state, &user, kb_id, Role::Admin).await?;
    if utopia_store::datasources::mounted(&state.pool, kb_id).await?.is_empty() {
        return Err(AppError::Validation("No data sources mounted".into()).into());
    }
    utopia_store::jobs::enqueue(&state.pool, "explore_mappings", json!({ "kb_id": kb_id }))
        .await?;
    Ok(Json(json!({ "ok": true })))
}
...
async fn sync_schema_doc(state: &AppState, kb_id: Uuid, ds_id: Uuid) -> anyhow::Result<usize> {
    ...
    for c in &cols {
        ...
        md.push_str(&format!(
            ...
            c.column,
            c.data_type,
            c.comment.as_deref().map(|x| format!(" — {x}")).unwrap_or_default()
        ));
    }
    ...
    .await?
    {
        ...
        None => {
            utopia_store::sources::create(
                &state.pool, kb_id, "folder", "Data schemas",
                &serde_json::json!({}), Some("database"), None, None,
            )
            .await?
            .id
        }
