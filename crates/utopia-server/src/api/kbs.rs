...
pub async fn list(
    ...
    utopia_store::workspaces::require_role(&state.pool, user.id, workspace_id, Role::Viewer).await?;
    let list =
...
pub async fn create(
    ...
    if !matches!(kind, "knowledge" | "memory") {
        return Err(
            AppError::Validation("kind must be 'knowledge' or 'memory'".into()).into(),
        );
    }
...
pub async fn update(
    ...
    let _ = utopia_store::audit::record(
        &state.pool, Some(id), user.id, "kb.updated", "kb", Some(id),
...
pub async fn delete(
    ...
    let _ = utopia_store::audit::record(
        &state.pool, None, user.id, "kb.deleted", "kb", Some(id),
        json!({ "name": kb.name }),
    ).await;
...
pub async fn set_member(
    ...
    kb_with_role(&state, &user, id, Role::Admin).await?;
    utopia_store::access::set_kb_member(&state.pool, id, user_id, &req.role, Some(user.id))
        .await?;
    let _ = utopia_store::audit::record(
        &state.pool, Some(id), user.id, "kb.member_set", "user", Some(user_id),
        json!({ "role": req.role }),
    ).await;
    Ok(Json(json!({ "ok": true })))
}
...
pub async fn remove_member(
    ...
    let _ = utopia_store::audit::record(
        &state.pool, Some(id), user.id, "kb.member_removed", "user", Some(user_id),
        json!({}),
    ).await;
    Ok(Json(json!({ "ok": true })))
}
