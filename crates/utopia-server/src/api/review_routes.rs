...
pub async fn close_fact(
    ...
    .fetch_optional(&state.pool)
    .await
    .map_err(|e| utopia_core::AppError::Db(e))?;
    if ok.is_none() {
    ...
    if let Some(mut d) = snap {
        ...
        let _ = utopia_store::audit::record(
            &state.pool, Some(kb_id), user.id, "fact.close", "fact", Some(fact_id), d,
        )
        .await;
    }
...
pub async fn resolve_conflict(
    ...
    let snap: Option<(String, Option<String>, String, Option<String>, String)> = sqlx::query_as(
...
pub async fn decide(
    ...
    if let Some((l, r, score)) = snap {
        let _ = utopia_store::audit::record(
            ...
            Some(kb_id),
            user.id,
            if body.action == "merge" { "review.merge" } else { "review.keep" },
            "review",
            Some(review_id),
...
pub async fn confirm_fact(
    ...
    if let Some(d) = snap {
        let _ = utopia_store::audit::record(
            &state.pool, Some(kb_id), user.id, "fact.confirm", "fact", Some(fact_id), d,
        )
        .await;
    }
    state.emit_review(kb_id);
...
pub async fn reject_fact(
    ...
    if let Some(d) = snap {
        let _ = utopia_store::audit::record(
            &state.pool, Some(kb_id), user.id, "fact.reject", "fact", Some(fact_id), d,
        )
        .await;
    }
    state.emit_review(kb_id);
