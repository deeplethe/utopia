...
pub async fn explore_mappings(state: &AppState, kb_id: Uuid) -> anyhow::Result<()> {
    ...
    for ds in &sources {
        let (engine, conn) =
            utopia_store::datasources::engine_and_conn(&state.pool, ds.id).await?;
    ...
    let prompt = format!(
        ...
        if existing_names.is_empty() { "(none)".into() } else { existing_names.join(", ") },
        schema_txt
    );

    let reply = client
        .chat(&[utopia_llm::ChatMessage { role: "user".into(), content: prompt }])
        .await?;
    let json_str = reply
        .trim()
    ...
    for p in proposals.iter().take(12) {
        ...
        let type_id: Option<(Uuid,)> = sqlx::query_as(
            "SELECT id FROM entity_types WHERE kb_id = $1 AND key = $2",
        )
        .bind(kb_id)
        .bind(kind)
        ...
        let resolved = utopia_store::resolution::resolve_mention(
            &state.pool, kb_id, type_id, name, None,
        )
        .await?;
        ...
        let mapped_to: Option<(Uuid,)> = sqlx::query_as(
            "SELECT id FROM relation_types WHERE kb_id = $1 AND key = 'mapped_to'",
        )
        .bind(kb_id)
        .fetch_optional(&state.pool)
        .await?;
        let Some((mapped_to,)) = mapped_to else { continue };
