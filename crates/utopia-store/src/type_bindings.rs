...
/// 已经有类的词喊着要建类。
pub async fn apply(pool: &PgPool, kb_id: Uuid, kind_word: &str, type_id: Uuid) -> AppResult<u64> {
    let sql = format!(
        "UPDATE entities
...
pub async fn unapply(pool: &PgPool, kb_id: Uuid, kind_word: &str) -> AppResult<u64> {
    let sql = format!(
        "UPDATE entities
