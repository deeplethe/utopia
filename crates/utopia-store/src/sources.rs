...
fn cron_next_after(expr: &str, after: DateTime<Utc>) -> Option<DateTime<Utc>> {
    ...
    schedule.after(&local_after).next().map(|t| t.with_timezone(&Utc))
}
...
pub async fn create(
    ...
    let interval = if cron_norm.is_some() { None } else { sync_interval_minutes };
    // serde 缺省的 Value::Null 会以 jsonb null 落库，前端读 config.x 直接炸——规范化为空对象
    let config = if config.is_null() { serde_json::json!({}) } else { config.clone() };
...
pub async fn document_exists_by_sha(
    pool: &PgPool,
    kb_id: Uuid,
    sha256: &str,
) -> AppResult<bool> {
...
mod tests {
    ...
    fn cron_validation() {
        ...
        assert_eq!(validate_cron("  0  9 * * Mon,Thu ").unwrap(), "0 9 * * Mon,Thu");
