...
//! （`llm_settings` 三把 key、`data_sources.conn_string`、`sources.config` 的凭据键、
...
pub async fn seal_llm_settings(pool: &PgPool, only: Option<Uuid>) -> AppResult<usize> {
    type Keys = (Uuid, Option<String>, Option<String>, Option<String>);
    let rows: Vec<Keys> = sqlx::query_as(
        "SELECT workspace_id, chat_api_key, embed_api_key, ocr_api_key FROM llm_settings
