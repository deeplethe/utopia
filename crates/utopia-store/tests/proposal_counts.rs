...
/// 造一个刚好能分辨两种口径的账本：
///
/// 说法 `acquired` 在**两篇**文档里都出现过，但只有一篇的那条事实还压在
/// 兜底谓词上——另一篇那条已经落到真关系上了（谓词匹配接住，或上一轮采纳搬走）。
///
/// 于是：积压口径 doc_count = 1（会被 `>=2` 门槛误杀），全量口径 = 2。
async fn seed(pool: &PgPool) -> anyhow::Result<Uuid> {
    let (org, ws, kb) = (Uuid::now_v7(), Uuid::now_v7(), Uuid::now_v7());
    let etype = Uuid::now_v7();
    let (fallback, real) = (Uuid::now_v7(), Uuid::now_v7());
    ...
    .execute(pool)
    .await?;
    // 兜底谓词的 key 必须跟 graph::FALLBACK_RELATION_KEY 一致，
    // 查询就是按它筛的
    for (id, key) in [(fallback, "related_to"), (real, "acquired_rel")] {
    ...
    // 一条还压在兜底谓词上，一条已经落到真关系上
    for (id, pred) in [(f_backlog, fallback), (f_moved, real)] {
        sqlx::query(
