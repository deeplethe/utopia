...
- **发版要把 compose 的固定标签跟上。** `docker-compose.yml` 里 `UTOPIA_IMAGE` 默认指向一个固定标签，README 那条「一条命令启动」跑的就是它。发一版的完整动作是：`dev → main` 合并、打标签发布，然后在 `dev` 上开一个 PR 把这个固定值改到新标签。rc6、rc7 发版时都漏了这一步，照 README 起来的人跑的是三周前的 rc5。rc8 起的标签是多架构的，固定值没指到多架构标签之前，arm64 主机拉不到默认镜像。
