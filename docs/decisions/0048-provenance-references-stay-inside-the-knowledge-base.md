...
- 2026-09-26 (#901): the family was pinned to migration 0070's table list, so every KB-owned table created afterwards escaped it — the catalog guard's scratch database stopped at 0070 and could not see post-0070 tables at all. Migration 0091 extends the same mechanisms to the post-0070 tables: composite same-KB foreign keys on `implication_rules`, `phrase_readings`, `errata_runs`, `errata_actions`, `attribute_rule_versions`, `derived_facts.attribute_rule_version_id`; an owner-derived trigger on `implied_fact_sources` (whose KB authority is the parent fact's `kb_id`); `UNIQUE (kb_id, id)` support on `implication_rules`, `attribute_rule_versions`, `errata_runs`; and `keep_their_kb` immutability on the five new KB-owned tables (`name_vectors` already carried it). `name_vectors`' two pre-existing composite edges and `attribute_rules.join_predicate_id` were also missing from the registered surface and export preflight — registry and scan now cover them. The catalog guard's scratch database now migrates through the current chain end, so future tables cannot become structurally invisible to the guard again.
