...
>
> So the row now says who drew its end. `end_derived` is true only on rows the engine closed. The text's own ends, a person's corrections and a person's conflict decisions keep it false, and are never recomputed. Any change to a timeline recomputes it in one pass: a value arrives, is observed again with new evidence, is merged in or reverted out, is rejected, or its document is deleted or restored. Each open or engine-closed row ends where the next different value with enough confidence begins, or stays open. The result depends only on which rows exist; the order they came in does not change it.
>
> A transaction that touches several timelines (a merge revert, a document deletion) takes all their advisory locks in a fixed order before it changes any row. Rows closed before this change keep `end_derived` false: the migration does not guess who closed them, and a base re-extracted afterwards follows the new rule.
>
