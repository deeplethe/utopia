...
- **Status**: implemented · `trino` / `databricks` / `snowflake` engines in `query_engine/` (migration `0020` widens the `engine` CHECK) · every engine is covered by protocol replays only (wiremock); **none has run against a real cluster** (#240, #241, #242) · MaxCompute is not done, see the last section
