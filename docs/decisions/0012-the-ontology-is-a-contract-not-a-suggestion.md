...
**状态**：已实施 · 五轮对照实验 · 违反率 57% → 4%，反向 39 → 0
...
上面全部在修**抽出来的东西对不对**。而「违反率 0%」可能是靠少抽换来的，
所以还要一个反向指标——**孤点**（一条事实都没有的实体）。

先排除了两个猜测。**不是分块的锅**：孤点的提及落在块末尾 20% 的占 14.7%，
有边的实体占 15.0%，位置分布几乎一样；如果是「关系被切到隔壁块」，孤点该明显偏向边缘。
**也不是模型编名字**：73 个孤点的名字在正文里**一次都找不到的是 0 个**，全部能逐字匹配。

真相是抽样看原文看出来的——**十条里七条是漏抽**：

    Vinod Khosla        "venture capitalist Vinod Khosla expressed interest in …"
    Grok-4              "xAI unveiled Grok-4"
    Satya Nadella       "Microsoft chief executive Satya Nadella" + "investing US$10 billion into OpenAI"
    UNESCO              "Hugging Face teamed up with Meta and UNESCO"
    Voxtral TTS         "Mistral AI released Voxtral TTS"

模型认出了实体、却没为它抽出那句话里明摆着的关系。而 `xAI unveiled Grok-4`
这种恰恰是最该抽的。

于是加了提示词规则 7a：**你列进 entities 的每一个东西，都该至少出现在一条事实里；
说不出关系就别把它列进去**。同语料同本体、60/60 块：

| | 事实 | 实体 | 孤点 | 空谓词 | 违反 |
|---|---|---|---|---|---|
| 无规则 7a | 427 | 417 | 73 = 17.5% | 199 | 0 |
| **有规则 7a** | 454 | 383 | 49 = **12.8%** | 250 | 0 |

**两头一起收**：实体少 34 个（不再列没话可说的东西），事实多 27 条（补上漏掉的关系）。
代价是空谓词 199 → 250——多抽的那些本体接不住，落成空谓词加原文说法。

**孤点是一把好尺，但它没解决根本问题**：12.8% 说明还有相当一部分在漏。
一行提示词收得了一些，收不完。

（中途 17 块时量到 6.0%，跑满是 12.8%——**中途快照不是结论**，这个教训这一轮犯过两次。）

## 待做

- **写入时的守卫挡不住写入之后的改动**：实体合并会把主语换成另一个类型的实体，
  于是事实「变成」违反。实测 6 条残留里 4 条如此。要治得把同样的检查放进合并路径。
- 另外 2 条残留（`Stability AI Ltd`、`Colossus 2 data center`）没有合并也没有改类型，
  **原因未查明**。
- 导入本体包时过滤掉需要中间节点才能用的关系（domain 全是 `Action`/`Offer`/
  `LoanOrCredit` 这类具化壳的），它们铺给模型只会产出违反。
