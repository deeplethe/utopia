...
superseded), low-confidence (`confidence < 0.75`), violations (`axiom_violations`) and defects
(`ontology_defects`), plus the Agent queue; ontology proposals sit on the ontology page and mappings
...
- **Queues organised by what a decision changes** [#725]: ontology proposals; alignment (a
  signature or rule the aligner cannot settle); time anchors (a missing or conflicting document
