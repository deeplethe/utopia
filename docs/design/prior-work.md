...
8. **A quote that is a substring is not yet support.** With frontier models 83 to 91% of required
   ...
   separate pass. *Open:* the judge's "misworded" splits into extrapolated and contradicted.
...
11. **Arguments before relations.** Galárraga et al. cluster noun phrases first and relation
    ...
    ingredient [Lairgi 2024]. Kind words bind before relation phrases [0044 cut 2]. *Open:* the
    phrase binding reads the classes at both ends, and the (class, class) signature is part of a
    phrase binding's key.
