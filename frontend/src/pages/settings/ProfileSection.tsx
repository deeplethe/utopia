...
export default function ProfileSection() {
  ...
  const saveName = async () => {
    ...
    try {
      ...
      await refresh()
      toast.success(name.trim() ? "Nickname updated" : "Nickname cleared")
    } catch (e) {
      toast.error(err(e))
    } finally {
      setSavingName(false)
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Your display name and password.</p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            A friendly nickname shown across the app. Your login username (
            <span className="font-medium">{user?.username}</span>) never changes.
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Enter your current password, then a new one (at least 6 characters).</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
