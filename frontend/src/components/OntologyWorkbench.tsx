...
function buildMaps(view: OntologyView): Maps {
  ...
  for (const p of view.data_properties) if (p.domain) push(dataByDomain, p.domain, p)
  ...
  for (const p of view.object_properties) {
    if (p.domain) push(objByDomain, p.domain, p)
    if (p.range) push(objByRange, p.range, p)
  }
  ...
  return { byIri, parentsOf, childrenOf, roots, dataByDomain, objByDomain, objByRange, disjointOf, equivOf }
}
 ...
 * hierarchy tree + the React Flow neighbourhood graph (Explore = focused 1-hop, Full = whole
 * ontology) + a detail inspector; the Table lens is the flat classes / properties / axioms browser.
    ...
    <div className="flex h-[calc(100svh-3.5rem)] min-h-[440px] flex-col">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b px-3 py-1.5">
        <Segmented
          ...
          options={[{ v: "graph", label: "Graph", icon: <Network className="h-3.5 w-3.5" /> },
                    { v: "table", label: "Table", icon: <Table2 className="h-3.5 w-3.5" /> }]}
        />
        {lens === "graph" && (
          <>
            <Segmented
              value={graphMode} onChange={setGraphMode}
              options={[{ v: "explore", label: "Explore" }, { v: "full", label: "Full Graph" }]}
            />
            {graphMode === "explore" && (
              <>
                <span className="ml-1 text-[11px] text-muted-foreground">层</span>
                <Segmented
          ...
          <div className="flex min-h-0 flex-col border-r bg-muted/20">
            <div className="border-b p-2">
              <div className="relative">
                ...
                <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search classes…" className="h-8 pl-7 text-sm" />
              </div>
            </div>
            <ScrollArea className="min-h-0 flex-1">
              <div className="p-1.5">
            ...
            <div className="border-t px-2 py-1.5 text-[11px] text-muted-foreground">
              {view.classes.length} classes · {maps.roots.length} roots
            </div>
          </div>

          {/* CENTER: React Flow graph (explore / full) */}
          <div className="relative min-h-0 bg-muted/40">
            <ReactFlowGraph view={view} maps={maps} focus={focus} selected={selected} mode={graphMode} depth={depth} onSelect={select} onExplore={drillIn} />
          </div>
            ...
            <ScrollArea className="h-full">
              {sel ? (
                <div className="space-y-4 p-4">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {canWrite && (
                        <div className="flex gap-1">
                          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => onEditClass(sel)}>
                  ...
                  <Relations title="Superclasses" iris={maps.parentsOf.get(sel.iri) ?? []} label={label} onSelect={explore} icon={<Layers className="h-3.5 w-3.5" />} />
                  <Relations title="Subclasses" iris={maps.childrenOf.get(sel.iri) ?? []} label={label} onSelect={explore} icon={<Layers className="h-3.5 w-3.5" />} />
                  <PropSection title="Object Properties (out)" rows={(maps.objByDomain.get(sel.iri) ?? []).map((p) => ({ k: p.label, v: p.range_label, iri: p.range }))} onSelect={explore} />
                  <PropSection title="Object Properties (in)" rows={(maps.objByRange.get(sel.iri) ?? []).map((p) => ({ k: `${p.domain_label ?? "?"} · ${p.label}`, v: sel.label, iri: p.domain }))} onSelect={explore} />
                  <PropSection title="Data Properties" rows={(maps.dataByDomain.get(sel.iri) ?? []).map((p) => ({ k: p.label, v: p.range_label ? p.range_label.replace(/^xsd:/, "") : null, iri: null }))} onSelect={explore} />
                  <Relations title="Disjoint" iris={maps.disjointOf.get(sel.iri) ?? []} label={label} onSelect={explore} icon={<Link2 className="h-3.5 w-3.5" />} />
                  <Relations title="Equivalent" iris={maps.equivOf.get(sel.iri) ?? []} label={label} onSelect={explore} icon={<Link2 className="h-3.5 w-3.5" />} />
                </div>
              ) : (
                <div className="p-4 text-sm text-muted-foreground">Nothing selected</div>
              )}
            </ScrollArea>
          </div>
        </div>
...
function TreeRow({
  ...
  const toggle = (e: MouseEvent) => {
    e.stopPropagation()
    setExpanded((s) => { const n = new Set(s); n.has(iri) ? n.delete(iri) : n.add(iri); return n })
  }
  return (
    <>
