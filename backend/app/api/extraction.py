...
from app.ontology import (
    abox_extract, conflict_agent, extract, schema, skos, store, structure_agent, tbox_reconcile,
    terminology_agent, terminology_sync, validation_agent,
)
...
async def _run_abox_extraction_job(
    ...
    with Session(engine) as session:
        ...
        async def progress(ev: dict) -> None:
            await _update_job_fields(job_id, processed_chunks=ev["index"] + 1)
...
async def _run_combined_extraction_job(
    ...
    with Session(engine) as session:
        ...
        async def prog_abox(ev: dict) -> None:
            await _update_job_fields(job_id, processed_chunks=n + ev["index"] + 1)
