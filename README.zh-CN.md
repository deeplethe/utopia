...
整套系统只有一个二进制文件和一个 Postgres，不需要 Elasticsearch，不需要向量数据库，也不需要消息队列。
...
**双时态图谱：**基于可编辑的本体进行 LLM 抽取，生成实体与事实。每条事实都保存 `valid_from` / `valid_to` 和对应证据行。修正事实时不会覆盖旧版本，而是闭合旧事实并链上新版本。图谱和邻域查询均接受 `at` 参数（`YYYY-MM-DD` 或 RFC3339），可以回读任意历史时点。实体面板同时展示两条时间线：一件事在现实中何时成立，以及系统何时形成这一判断、又何时改变判断。
...
**基于本体的智能问数：**在系统层注册一次 Postgres 连接，再挂载到知识库，对话就可以同时查询文档和数据库。
