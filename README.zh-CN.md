...
Utopia 能解决什么问题，按人带着来的那个问题来说。除注明外都是现在就能用的；每一条都需要配一个对话模型，拿不准的地方都留给人在审核里定。

| 谁、什么问题 | 输入 | 问什么，得到什么 | 比检索和 RAG 多出来的 |
|---|---|---|---|
| **合同链。** 法务或采购，一份主协议被多次修订。 | 主协议和历次修订，PDF 或 Word。 | *2024 年 3 月 15 日适用的付款条款是什么，当事方是谁？* 那一天生效的条款、它来自哪次修订、原句。 | 每条事实带着它成立的时间和出处；后来的修订关闭前一条，而不是覆盖。[合同语料](scripts/bench/corpora/contracts/README.md) 就是这个场景，附带它必须答对的问题。 |
| **谁在什么时候担任什么。** 人事、财务、科研里会变的记录：交接、更正、重述。 | 公告、纪要、申报文件，按写下的日期。 | *2023 年二季度谁负责 X 项目，当时我们对它的认识是什么？* 那段时间的负责人，以及分开给出的、当时库里知道的内容。 | 两根时间轴：事情何时成立，库何时得知。一次更正链回它替换的那条。[时态问答 bench](scripts/bench/README.md) 量的就是这个。 |
| **读库的智能体。** 用 Claude Desktop、Cursor 或自建 agent 的工程、运维团队。 | 已有的知识库；agent 用限定到这个库的令牌通过 MCP 接入。 | *某实体在某天的事实；两个东西之间的路径；上周变了什么。* 带稳定标识和证据的结构化答案；agent 想写入的东西先交给人。 | 读到的是带类型和日期的事实，不是段落；写入是提议。[MCP 指南](web/src/docs/mcp.md) 列了全部工具。 |
| **来自系统的观测。** 机器人、监控、安全工具：系统自己知道看见了什么、在什么时候。 | 一次观测一次推送，直接用抽取契约，带观测时间。rc7 新增。 | *这个物体 08:05 在哪，现在在哪？* 那一刻成立的值，以及它所依据的那次观测。 | 系统和图之间没有模型；函数型属性在后一次观测到来时关闭前一个值。[走通示例](docs/walkthrough.md) 用 `curl` 把这条路从头跑到尾。 |

走通示例是看完整路径最短的办法：样例数据、摄入、提问、带证据的回答，以及这个库做不到的事。
