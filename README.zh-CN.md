...
所以，我们给它取了一个稍显浪漫的名字：**乌托邦（Utopia）**。如果一定要给它一个工程上的描述，我们更愿意称它为：一个被动演化的企业世界模型。它不凭空创造世界，而是持续观察现实、吸收事实、记录变化，在一次次事件发生之后重新理解自己所处的世界。记忆、知识、规则、时间、推理与行动，都只是构成这个世界的一部分。
